...
import { Monitor, MonitorCheck, MonitorStatus } from "@/generated/prisma/client";
...
const StatusBadge = ({ status }: {
  ...
  return (
    <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold tracking-wide border ${colors.bg} ${colors.text} ${colors.border}`}>
      <span className={`relative flex h-2 w-2`}>
        <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${colors.ping}`}></span>
        <span className={`relative inline-flex rounded-full h-2 w-2 ${colors.solid}`}></span>
      </span>
      {status || 'UNKNOWN'}
    </div>
  );
...
const MetricItem = ({ icon: Icon, label, value, subValue, colorClass = "text-slate-700" }: {
  icon: LucideIcon,
  label: string,
  value: string,
  subValue?: string,
...
const MonitorCard = ({ monitor }: {
  ...
  const status: MonitorStatus | 'UNKNOWN' = monitor.lastCheck?.status || 'UNKNOWN';
      ...
      <div className="p-6">
        ...
        <div className="flex justify-between items-start mb-6">
          <div className="flex gap-4">
            <div className={`p-3 rounded-xl flex items-center justify-center h-12 w-12 ${statusColors[status].bg} ${statusColors[status].text}`}>
              <Server size={24} />
            </div>
            <div>
              <div className="flex items-center gap-3 mb-1">
                <h3 className="font-bold text-slate-800 text-lg">{monitor.name}</h3>
          ...
          {monitor.lastCheck && (
            <div className={`hidden sm:flex flex-col items-end`}>
              <span className={`text-2xl font-black opacity-20 ${status === 'UP' ? 'text-slate-400' : status === 'DOWN' ? 'text-rose-400' : 'text-slate-400'}`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 py-6 border-t border-b border-slate-100 bg-slate-50/50 rounded-xl px-4 mb-6">
          <MetricItem
            icon={Zap}
            label="Latency"
            value={monitor.lastCheck ? monitor.lastCheck.responseTimeMs.toString() : "N/A"}
            colorClass={
              status === 'UP'
                ? (monitor.lastCheck && monitor.lastCheck.responseTimeMs < 200 ? "text-emerald-600" : "text-amber-600")
                : status === 'DOWN'
                ? "text-rose-600"
                : "text-slate-600"
            }
          ...
          <MetricItem
            icon={Clock}
            label="Last Check"
            value={monitor.lastCheck ? formatTimeAgo(monitor.lastCheck.createdAt) : "Never"}
          />
          <MetricItem
            icon={status === 'UP' ? CheckCircle2 : status === 'DOWN' ? AlertCircle : Clock}
            label="Status Since"
